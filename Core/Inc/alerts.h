...
extern Alert_S* statusAlerts[];

// Function pointer types for function interators
typedef bool (*telemetryAlertCondition)(telemetryTaskData_S* telemetryData);
typedef bool (*statusAlertCondition)(statusUpdateTaskData_S* statusData);

// Function arrays
extern telemetryAlertCondition telemetryAlertConditionArray[];
extern statusAlertCondition statusAlertConditionArray[];

// The total number of alerts for each alert task
extern const uint32_t NUM_TELEMETRY_ALERTS;
extern const uint32_t NUM_STATUS_ALERTS;
